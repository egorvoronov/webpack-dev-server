...
function Server(compiler, options) {
	...
	compiler.plugin("done", (stats) => {
    	if(options.manualUpdate) {
    		timeout = setTimeout(() => this._sendStats(this.sockets, stats.toJson(clientStats)), 10000);
    	} else {
		    this._sendStats(this.sockets, stats.toJson(clientStats));
    	}
		this._stats = stats;
	});
	options.manualUpdate && options.manualUpdate.on('change', () => {
		clearTimeout(timeout);
		this._sendStats(this.sockets, { errors: [], warnings: [] }, true); // send force update due to manual update
	});
